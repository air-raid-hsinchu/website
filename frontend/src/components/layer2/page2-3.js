...
import mobileMapImg from "../../assets/images/2-3 地圖原圖.png";
...
const MapPin = ({ offset, size, data, field, isHover, infoPosition, isMobile, overflow=false }) => {
    const pinSize = 10;
    const title_size = isMobile ? "0.8rem" : "1.5rem";
    const text_size = isMobile ?  "0.6rem" : "1.2rem";
        ...
        <Transition in={isHover} timeout={500}>
            <>
                ...
                <div
                    ...
                    style={{
                        left: offset.x + pinSize + (overflow ? -70 : 5),
                        top: offset.y - size.height / 2 + pinSize / 2 + (overflow ? 20 : 0),
                        textAlign: 'center',
                        width: size.width,
                        height: size.height,
                        position: 'absolute',
                ...
                {(isHover && data.date && data.description) && (
                    <div
                        style={{
                            width: window.innerWidth * (isMobile ? 0.2 : 0.21),
                            height: window.innerHeight * 0.3,
                            position: 'absolute',
                            bottom: -infoPosition.y + window.innerHeight + 75,
                            right: -infoPosition.x + (isMobile ? window.innerWidth * 0.6 : 0) + (overflow ? 90 : 0),
                            color: 'black',
                        }}
                    >
                        <div
        ...
        <>
            {isMobile ? (
                <Fade in={true} timeout={fadeInOptions.timeout}>
                    <div className='background d-flex align-items-start justify-content-center'>
                        <div
                            ref={mapRef}
                            className='map'
                            style={{
